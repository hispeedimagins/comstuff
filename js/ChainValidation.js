...
function validate_account_name(value) {
    ...
    if (!value) {
        return 'chainvalidation_js.account_name_should_not_be_empty';
    }
    length = value.length;
    if (length < 3) {
        return ('chainvalidation_js.account_name_should_be_longer');
    }
    if (length > 16) {
        return ('chainvalidation_js.account_name_should_be_shorter');
    }
    // if (BadActorList.includes(value)) {
    //     return ('chainvalidation_js.badactor');
    // }
    ref = value.split('.');
